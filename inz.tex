...
\chapter{Organizacja pracy}
	...
	\section{Przebieg prac}
		Projekt realizowany był etapami. Zostały one opisane poniżej.
		
		\subsection{Określenie wymagań sprzętowych}
			Propozycja wykorzystania układu RP2040 była względnie ryzykowna. Rdzenie Cortex M0+ mogły okazać się niewystarczające, zwłaszcza, że brak im jednostki obliczeń zmiennoprzecinkowych. Należało więc jak najszybciej określić czy poradzą sobie one z najważniejszym i najbardziej wymagającym zadaniem jakim jest dekodowanie formatu MP3.
			
			\imgh{4/prototype}{Pierwszy prototyp do testów odtwarzania dźwięku}{0.6}
			
			W krótkim czasie powstał prototyp przedstawiony na rysunku~\ref{4/prototype}. Zmontowany na płytce stykowej, zawierał minimum elementów wymaganych do testów audio. Znalazły się tam układy przetwornika cyfrowo-analogowego i wzmacniacza, oraz slot na kartę SD.
...
%			Jednak do kompletnego produktu brakowało jeszcze interfejsu użytkownika.
		...
		\subsection{Interfejs użytkownika}
			Koniec czerwca to implementacja architektury ekranów użytkownika. Na początku została dodana obsługa ekranów klawiatury, wyników wyszukiwania i odtwarzacza. Wraz z końcem miesiąca radio było gotowe do znajdowania i odtwarzania stacji.
		...
		\subsection{Schemat, płytka i obudowa}
			Gdy wymagane funkcje były już gotowe przystąpiono do stworzenia finalnego urządzenia na podstawie wyżej wspomnianego prototypu wzbogaconego o ekran i przyciski, przedstawionego na rysunku~\ref{4/prototype_2}.
			...
			\imghss{4/pcb}{Płytka PCB po wytrawieniu, początek procesu wiercenia otworów}{4/whole}{Kompletny prototyp na płytce}
			
			Po przetestowaniu kompletnego prototypu zmontowanego na płytce PCB (rysunek~\ref{4/whole}), zaprojektowana została obudowa. Wydrukowana została na drukarce 3D. Panel frontowy pokazany został na rysunku~\ref{4/case1}.
			$ $\\
		...
		\subsection{Odtwarzanie plików lokalnych}
			Jako że obsługa kart SD była wymaganiem opcjonalnym, zrealizowana została pod koniec prac nad urządzeniem. We wrześniu został dodany ekran listowania plików. Za ekran wykonawczy posłużył ekran radia z pomniejszymi zmianami. Dodano pasek postępu informujący użytkownika jaka część utworu została już odtworzona. Znalazło się też miejsce na informacje o czasie trwania całości. Oba ekrany przedstawiono na rysunkach \ref{4/scr_1} oraz \ref{4/scr_2}.
			
			\imghss{4/scr_1}{Ekran radia}{4/scr_2}{Ekran odtwarzacza lokalnego}
		...
		\subsection{Automatyczne połączenie Wi-Fi}
			Funkcjonalność niewymagana w żaden sposób do testów była gotowa w pierwszej połowie grudnia. Do tej pory jedyna sieć Wi-Fi była zapisana na stałe w kodzie programu. Dodana została funkcjonalność połączenia się z dowolną siecią. Powstała także lista zapisanych sieci, z której przy starcie urządzenie automatycznie wybiera najlepszą sieć do połączenia się.
			
	% TODO instrukcja obsługi
