...
%		Nawigacja oparta jest na siatce przycisków, przy czym może być ona nieregularna (różna liczba przycisków w różnych wierszach). 
	...
	W tym rozdziale wyjaśnione zostało jak funkcjonuje powstały projekt.
	
	\section{Moduły projektu}
	W kodzie starano się zachować porządek. Wydzielone zostały główne funkcjonalne elementy i na podstawie tego podziału stworzona została struktura modularna przedstawiona poniżej.
	
	\subsection{Moduły własne}
	Większość kodu projektu zawiera katalog \lstinline|libs|. W nim znajdują się poszczególne moduły. Wszystkie wypisane w tej sekcji zostały zaimplementowane samodzielnie, chyba, że zaznaczono inaczej. Dla przejrzystości pogrupowano moduły wg ich ról.
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \lstinline|radios| -- obsługa list jak i ich ładowanie (wyszukiwanie/ulubione/listing plików na karcie SD)
			\item \lstinline|screens| -- widoki użytkownika
			\item \lstinline|tft| -- niskopoziomowa obsługa ekranu ze sterownikiem ST7735s. Kod inicjalizacji i przesyłania pikseli zaczerpnięty z Adafruit \ref{?}
		\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \lstinline|helixmp3| -- biblioteka dekodująca format MP3\textsuperscript{\cite{helixmp3}}
		\item \lstinline|littlefs| -- system plików LittleFS\textsuperscript{\cite{littlefs}}
		\item \lstinline|sd_lib| -- biblioteka FatFS \ref{?} wraz z kodem dostępu do karty sd\textsuperscript{\cite{sdfs}}
	\end{itemize}
	...
	Odtwarzanie dźwięku odbywa się za pomocą zewnętrznego przetwornika DAC. Mikrokontroler wysyła dźwięk w formacie PCM protokołem \isqs{} do specjalizowanego układu. Drugi rdzeń procesora wykorzystywany jest do dekompresji formatów audio.
	...
	\paragraph{DMA}
		...
		Podsystem bezpośredniego dostępu do pamięci wysyła dane do maszyn stanu PIO. Wykorzystane zostały dwa kanały, aby móc prowadzić nieprzerwaną transmisję. Schemat działania przedstawiony został na rysunku \ref{3/dma_chain}. W momencie gdy jeden kanał skończy transfer, automatycznie uruchamia drugi kanał. Jest to sprzętowa funkcja RP2040. Wywoływane jest także przerwanie, które zgłasza procesorowi potrzebę załadowania nowych danych.
		
	\paragraph{PIO}
		RP2040 posiada programowalne maszyny stanów. Programowane są one własnościowym językiem z rodziny assembler. Dzięki tym blokom, możliwe jest dodanie do mikrokontrolera dowolnego protokołu danych bez zbytniego obciążenia procesora jego pełną symulacją. W ramach tej pracy został stworzony blok obsługi interfejsu \isqs{}, który serializuje dane dostarczane przez DMA i wysyła do układu DAC.
	$ $\\
	
	Wszystko to dzieje się na rdzeniu 0. Taka koncepcja pozwala na odtwarzanie plików WAV (jeżeli mamy je jak załadować do pamięci). Problem powstaje, gdy musimy w czasie rzeczywistym zdekodować nietrywialny format zapisu audio, np. MP3.
