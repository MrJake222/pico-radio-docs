...
%		Poniżej zostały przedstawione wymagane funkcjonalności, jak również te dodatkowe.
		...
		\subsection{Wymagania niefunkcjonalne}
			Urządzenie ma być zbudowane z~wykorzystaniem mikrokontrolera lub płytki rozwojowej opartej o~mikrokontroler.
...
%			\imgh{3/hw_kicad_sch}{Schemat elektryczny radia}{1}
			...
			\subsubsection{Sekcja zasilania}
				Rysunek~\ref{3/hw_kicad_sch_power} przedstawia schemat sekcji zasilania.
...
%			 	Składa się on głównie z~płytki rozwojowej Raspberry Pi Pico~W, która zawiera układ mikrokontrolera RP2040 oraz pamięć oprogramowania układowego typu FLASH. Znajduje się tam także moduł komunikacji bezprzewodowej Wi-Fi.
		...
		\subsection{Obudowa}
			...
			Na rysunku~\ref{3/hw_scad_case} przedstawiono wizualizację projektu całej obudowy. Poniżej, na rysunku~\ref{3/hw_case}, zaprezentowano efekt finalny, funkcjonalne radio w~obudowie.
	...
	\section{Struktura oprogramowania układowego}
		...
		\subsection{Moduły własne}
			Większość kodu projektu zawiera katalog \lstinline|libs|. W~nim znajdują się poszczególne moduły (opisane odpowiednimi nazwami podfolderów). Wszystkie wymienione w~tej sekcji zostały zaimplementowane samodzielnie, chyba, że zaznaczono inaczej. Dla przejrzystości pogrupowano moduły wg ich ról.
...
%			\imgcs{3/pipeline}{Koncepcja wielowątkowego dekodowania}{0.8}
	...
	%	TODO opisać tutaj co robi player.cpp (zmienna dec, funkcje publcizne)
		...
		\subsection{PIO}
			...
			W~ramach pracy został stworzony blok obsługi interfejsu \isqs{}, który serializuje dane dostarczane przez DMA i~wysyła do układu DAC.
			$ $\\
...
%			\noindent
			Kod bloku PIO z~listingu~\ref{lst/3/i2s.pio} odpowiada za realizację transmisji w~standardzie \isqs{} ilustrowanej przebiegiem pokazanym na rys~\ref{3/pio_i2s_docs}.
			
			\lstfile{pio}{Kod programu \isqs{}}{lst/3/i2s.pio}
			
			\noindent
			Przedstawiony program działa następująco:
			\begin{itemize}
		...
		\subsection{DMA}
			...
			Aby zachować płynność odtwarzania, wykorzystano podwójne buforowanie\dmacite{}. Na rysunku~\rdmabuf{} przedstawiono schematy blokowe ilustrujące jego działanie.
			...
			Ukończenie przesyłania drugiej części bufora przebiega analogicznie. Uruchamiany jest pierwszy kanał DMA, a~procesor uzupełnia drugą część bufora. Sytuację przedstawia rysunek~\rdmabuf{d}.
			$ $\\
...
%			Na uznanie zasługuje sugestia z~listy mailingowej projektu LwIP.
		...
		\subsection{Ostateczna wersja prototypu}
			...
			\begin{itemize}
				...
				\item Do wzmacniacza doprowadzony został sygnał wyciszający, co pozwoliło zmniejszyć uciążliwość szumów pochodzących od niedostatecznie filtrowanego napięcia zasilającego. Zakłócenia te były szczególnie słyszalne przy braku innych dźwięków.
			\end{itemize}
...
%	skupiono się na finalnym efekcie projektu.
	...
	\section{Przegląd zrealizowanych funkcji}
		...
		\subsection{Lista ulubionych stacji}
			...
			Jest to także ekran startowy. Wyświetla się on jako pierwszy po uruchomieniu urządzenia. Dlatego jest też traktowany jako ekran główny i~posiada najwięcej ikon akcji na dole ekranu.
...
%			Tworzony jest on na podstawie plików znajdujących się na karcie SD.
		...
		\subsection{Połączenie z~Internetem}
			Do odtwarzania radia i~wyszukiwania nowych stacji urządzenie wymaga połączenia z~Internetem. W~aktualnej wersji oprogramowania zostały zaimplementowane funkcje wyszukiwania, zapisywania oraz łączenia się z~sieciami Wi-Fi.
			$ $\\
	...
	\section{Scenariusze użytkowania}
		...
		\subsection{Wyszukanie i~zapisanie stacji}
			...
			\begin{itemize}
				...
				\item Rys.~\rfladd{c} -- na trzeciej stronie odnaleziona została żądana stacja
		...
		\subsection{Połączenie z~nową siecią Wi-Fi}
			...
			Na rysunku~\rflwifi{} przedstawiono scenariusz łączenia się z~nową siecią Wi-Fi i~dodanie jej do listy znanych sieci.
			...
			Ta sekcja zawiera listę niedociągnięć, których wyeliminowanie umożliwiłoby w~przyszłości stworzenie lepszego produktu.
				...
				\begin{itemize}
					...
					\begin{itemize}
						...
						\item nieprecyzyjne rezystory pomiarowe w~dzielnikach napięć powodują niedokładności w~pomiarze napięć \lstinline|VCC| i~ogniw,
					\end{itemize}
...
%				\end{itemize}
				...
				Metoda termotransferu nie zapewnia odpowiedniej jakości wykonania płytki. Niektóre ścieżki są cieńsze niż w projekcie, a~część padów lutowniczych jest tak cienka, że montaż był znacząco utrudniony. Lepszym i~powtarzalnym sposobem na wytwarzanie płytek drukowanych byłoby zlecenie ich wykonania zewnętrznej firmie.
		
		\subsection{Radio nowej generacji}
			Pierwszym ulepszeniem powinno być skorygowanie schematu oraz stworzenie nowej płytki, z~zaprojektowaną od podstaw mozaiką ścieżek. Wykonana profesjonalnie, dwuwarstwowo, pozwoliłaby na poprawienie parametrów pracy, głównie obniżenie szumów sekcji zasilającej.
			Należy odpowiednio rozmieścić otwory montażowe służące zamocowaniu płytki w~obudowie.
			...
			Należałoby się zastanowić także, czy jakość i poziom głośności dźwięku odpowiada użytkownikowi. Jeżeli nie, istnieje możliwość wykorzystania mocniejszego wzmacniacza, który przy cichym słuchaniu będzie oferował mniejszy poziom zniekształceń lub pozwoli odtwarzać dźwięk z~większą mocą. 
			Należy jednak mieć na uwadze, że układy o~większej mocy wymagają wyższego napięcia zasilającego.
			Przykładowo PAM8610\textsuperscript{\cite{ch5_pam8610}} przy napięciu 7V (przybliżone napięcie 2~ogniw połączonych szeregowo) jest w~stanie oddać moc 4W na kanał (obecnie użyty wzmacniacz posiada moc ok. 1W na kanał). Nowszy PAM8006a\textsuperscript{\cite{ch5_pam8006a}} jest w~stanie wysterować głośniki mocą aż 8W na kanał, lecz wiąże się to z~podwyższeniem napięcia zasilania do 12V (3 ogniwa szeregowo) i~zmianą głośników na takie o~impedancji $8 \Omega$.
